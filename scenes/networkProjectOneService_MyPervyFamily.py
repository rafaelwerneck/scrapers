...
class ProjectOneServiceSpider(BaseSceneScraper):
    name = 'ProjectOneServiceMyPervyFamily'
    network = 'mindgeek'
    ...
    def start_requests(self):
        ...
        for url in self.start_urls:
            yield scrapy.Request(url=url, callback=self.parse, meta={'url': url, 'start_url': url}, headers=self.headers, cookies=self.cookies)
    ...
    def parse(self, response):
        ...
        headers = {
            'instance': token,
            'origin': response.meta['start_url'],
            'referer': response.meta['start_url'],
        }
    ...
    def get_scenes(self, response):
        ...
        for scene in response.json()['result']:
            # print(scene['brand'].lower().strip(), self.parse_date(scene['dateReleased']).strftime('%Y-%m-%d'))
            if "levi" in scene['brand'].lower().strip() and "productions" in scene['brand'].lower().strip():
                item = SceneItem()
                item['date'] = self.parse_date(scene['dateReleased']).strftime('%Y-%m-%d')
                if item['date'] > "2025-10-23": # When it moved from AdultTime to Project1Service
